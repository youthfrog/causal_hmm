...
def test(args, causal_hmm_model, classifier, test_dataset, batch_size, writer, epoch, n_iter, test_log):

    test_loader = DataLoader(test_dataset, batch_size=batch_size, num_workers=4, drop_last=False, shuffle=False)
    ...
    with torch.no_grad():
        ...
        for i_batch, (sample_batched) in enumerate(test_loader):
            ...
            writer.add_scalar('test/mean rec loss of image', mean_rec_loss, n_iter)
            writer.add_scalar('test/mean kl loss of image', mean_kl_loss, n_iter)
            writer.add_scalar('test/mean rec loss of A', mean_rec_loss_A, n_iter)
            writer.add_scalar('test/mean kl loss of A', mean_kl_loss_A, n_iter)
            ...
            writer.add_scalar('test/classification loss', loss_pre, n_iter)
        ...
        all_test_auc = auc(fpr, tpr)
        all_test_acc = (all_pre_id == all_gt_y).sum() / len(all_gt_y)
        writer.add_scalar('test/AUC', all_test_auc, epoch)
        writer.add_scalar('test/ACC', all_test_acc, epoch)
        test_log['test_log'].info("test acc:{0}, test auc:{1} at epoch {2}".format(all_test_acc, all_test_auc, epoch))
